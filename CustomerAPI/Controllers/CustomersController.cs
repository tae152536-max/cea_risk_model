...
public class CustomersController : ControllerBase
{
    ...
    private async Task<IActionResult> FromGoogleFormInner(GoogleFormPayload payload)
    {
        if (string.IsNullOrWhiteSpace(payload.DrName) ||
            ...
            return BadRequest("DrName and Hospital are required.");

        await using var cn = new SqlConnection(_conn);
        await cn.OpenAsync();

        // Resolve MedRepID — prefer direct ID, fall back to email lookup
        int medRepId = payload.MedRepID > 0 ? payload.MedRepID
            : string.IsNullOrWhiteSpace(payload.MedRepEmail) ? 0
            : await LookupMedRepAsync(cn, payload.MedRepEmail);
        if (medRepId == 0)
            return BadRequest($"MedRep not found. Ensure you are logged in correctly.");

        // Resolve AreaID — prefer direct ID, fall back to name lookup
        int areaId = payload.AreaID > 0 ? payload.AreaID
            : await LookupAreaAsync(cn, payload.Area);
        if (areaId == 0)
